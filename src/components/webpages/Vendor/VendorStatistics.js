    ...
    useEffect(() => {
        if (user_type() === 'vendor') {
            async function fetchData() {
                const response_1 = await axios.get('http://localhost:5000/orders/vendor', {
                    headers: {
                        authorization: localStorage.getItem('token')
                    }
                });
                const response_2 = await axios.get('http://localhost:5000/items', {
                    headers: {
                        authorization: localStorage.getItem('token')
                    }
                });
                const response_3 = await axios.get('http://localhost:5000/vendors/details', {
                    headers: {
                        authorization: localStorage.getItem('token')
                    }
                });
                const response_4 = await axios.get('http://localhost:5000/buyers', {
                    headers: {
                        authorization: localStorage.getItem('token')
                    }
                });
